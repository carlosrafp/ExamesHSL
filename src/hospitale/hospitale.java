...
public class hospitale {
    private Map<String, String> sntCookies;
    private JSONArray listaPacientes;

    public hospitale (){
        this.listaPacientes = new JSONArray();
        //this.sntCookies = new Map<String,String>();
    }

    public int fazerLogin(String login, String senha) throws Exception{
        //etapas para login
        ...
        this.sntCookies = res.cookies();
        this.sntCookies.remove("Eclipse"); // remove Eclipse cookie porque nao fez login ainda
        ...
        String viewStateStr = Utils.strings.escapeStr(viewState.val());
        ...
        return 0; // sucesso login
    }

    public int buscarListaPacientes () throws Exception{
        ...
        Connection.Response res = Jsoup.connect("http://192.168.245.169/hospitaleHSL/Clinico/ProntuarioEletronico/p_hosp_1401_PEP.aspx")
                .userAgent("Mozilla/5.0")
                .ignoreContentType(true)
                .ignoreHttpErrors(true)
                .followRedirects(false)
        ...
        Elements viewState = doc.select("input[name=__VIEWSTATE]");
        Elements viewStateGenerator = doc.select("input[name=__VIEWSTATEGENERATOR]");
        String viewStateStr = strings.escapeStr(viewState.val());
        ...
        viewState = doc.select("input[name=__VIEWSTATE]");
        viewStateGenerator = doc.select("input[name=__VIEWSTATEGENERATOR]");
        viewStateStr = strings.escapeStr(viewState.val());
        ...
        //this.listaPacientes = new JSONArray();  // prepara JSON array com pacientes
        for (Element paciente : pacientes){
            ...
            this.listaPacientes.put(pac);  // acrescenta object json do paciente na lista
        }

        return 0;
    }
    public JSONArray getListaPacientes(){
        return new JSONArray(this.listaPacientes);
    }

    public String getListaPacientesStr(){
        return this.listaPacientes.toString();
    }
